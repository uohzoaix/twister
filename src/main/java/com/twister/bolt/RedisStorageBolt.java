...
public class RedisStorageBolt extends BaseRichBolt {
	...
	public void execute(Tuple input) {
		...
		while (iter.hasNext()) {
			...
			//如果需要可以重组下级key
			logalys.setKey(ukey);
			// from reids json build object
			AccessLogAnalysis clog=amc.restoreObjectFromJson(ukey,AccessLogAnalysis.class);	
			if (clog!=null && clog.getKey().equals(ukey)){
				logalys=logalys.calculate(clog); //在对象里算  
				
			}
			amc.storeObjectToJson(ukey,logalys); //to redis json
