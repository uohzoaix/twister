...
/**
 * A little-endian Java heap buffer. It is recommended to use
 * {@link ChannelBuffers#buffer(ByteOrder, int)} and
 * {@link ChannelBuffers#wrappedBuffer(ByteOrder, byte[])} instead of calling
 * the constructor explicitly.
 */
public class LittleEndianHeapChannelBuffer extends HeapChannelBuffer {
	
	/**
	 * Creates a new little-endian heap buffer with a newly allocated byte
	 * array.
	 * 
	 * @param length
	...
	public int getUnsignedMedium(int index) {
		return array[index] & 0xff | (array[index + 1] & 0xff) << 8 | (array[index + 2] & 0xff) << 16;
	}
	...
	public long getLong(int index) {
		return (long) array[index] & 0xff | ((long) array[index + 1] & 0xff) << 8
	...
	public ChannelBuffer copy(int index, int length) {
		if (index < 0 || length < 0 || index + length > array.length) {
			throw new IndexOutOfBoundsException("Too many bytes to copy - Need " + (index + length) + ", maximum is "
					+ array.length);
		}
