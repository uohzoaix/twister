 ...
 * deallocated less efficiently than an ordinary heap buffer. Consequently, a
 * user will get {@link OutOfMemoryError} when one tries to allocate small
 * direct buffers more often than the GC throughput of direct buffers, which is
 * much lower than the GC throughput of heap buffers. This factory avoids this
 * problem by allocating a large chunk of pre-allocated direct buffer and
...
public class DirectChannelBufferFactory extends AbstractChannelBufferFactory {
	...
	private static final DirectChannelBufferFactory INSTANCE_LE = new DirectChannelBufferFactory(
	...
	public DirectChannelBufferFactory(ByteOrder defaultOrder, int preallocatedBufferCapacity) {
		...
		if (preallocatedBufferCapacity <= 0) {
			throw new IllegalArgumentException("preallocatedBufCapacity must be greater than 0: "
					+ preallocatedBufferCapacity);
		}
