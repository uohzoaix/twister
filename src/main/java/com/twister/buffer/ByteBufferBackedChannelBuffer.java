 ...
 * {@link ChannelBuffers#directBuffer(int)} and
 * {@link ChannelBuffers#wrappedBuffer(ByteBuffer)} instead of calling the
...
public class ByteBufferBackedChannelBuffer extends AbstractChannelBuffer {
	...
	public void getBytes(int index, byte[] dst, int dstIndex, int length) {
		...
		try {
			...
			throw new IndexOutOfBoundsException("Too many bytes to read - Need " + (index + length) + ", maximum is "
					+ data.limit());
		}
	...
	public void getBytes(int index, ByteBuffer dst) {
		...
		try {
			...
			throw new IndexOutOfBoundsException("Too many bytes to read - Need " + (index + bytesToCopy)
					+ ", maximum is " + data.limit());
		}
		dst.put(data);
	}
	...
	public ChannelBuffer slice(int index, int length) {
		if (index == 0 && length == capacity()) {
			...
			return new ByteBufferBackedChannelBuffer(((ByteBuffer) buffer.duplicate().position(index)
					.limit(index + length)).order(order()));
		}
	}
