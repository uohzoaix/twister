...
/**
 * Creates a new {@link ChannelBuffer} by allocating new space or by wrapping or
 * copying existing byte arrays, byte buffers and a string.
 * 
 * <h3>Use static import</h3> This classes is intended to be used with Java 5
 ...
 * <li>{@link #dynamicBuffer(int)} allocates a new dynamic-capacity heap buffer,
 * whose capacity increases automatically as needed by a write operation.</li>
 * </ul>
 * 
 * <h3>Creating a wrapped buffer</h3>
 * 
 * Wrapped buffer is a buffer which is a view of one or more existing byte
 * arrays and byte buffers. Any changes in the content of the original array or
 * buffer will be visible in the wrapped buffer. Various wrapper methods are
 * provided and their name is all {@code wrappedBuffer()}. You might want to
 * take a look at the methods that accept varargs closely if you want to create
 ...
 * buffers or a string. Unlike a wrapped buffer, there's no shared data between
 * the original data and the copied buffer. Various copy methods are provided
 * and their name is all {@code copiedBuffer()}. It is also convenient to use
 * this operation to merge multiple buffers into one buffer.
 * 
 * <h3>Miscellaneous utility methods</h3>
 * 
 * This class also provides various utility methods to help implementation of a
 * new buffer type, generation of hex dump and swapping an integer's byte order.
 * 
 * @apiviz.landmark
...
public final class ChannelBuffers {
	...
	/**
	 * Creates a new Java heap buffer with the specified {@code endianness} and
	 * {@code capacity}. The new buffer's {@code readerIndex} and
	 * {@code writerIndex} are {@code 0}.
	 */
	 ...
	 * length. More accurate estimation yields less unexpected reallocation
	 * overhead. The new buffer's {@code readerIndex} and {@code writerIndex}
	 * are {@code 0}.
	 */
	...
	/**
	 * Creates a new dynamic buffer with the specified endianness and the
	 * specified estimated data length. More accurate estimation yields less
	 * unexpected reallocation overhead. The new buffer's {@code readerIndex}
	...
	/**
	 * Creates a new big-endian dynamic buffer with the specified estimated data
	 * length using the specified factory. More accurate estimation yields less
	 * unexpected reallocation overhead. The new buffer's {@code readerIndex}
	 * and {@code writerIndex} are {@code 0}.
	 */
	...
	/**
	 * Creates a new dynamic buffer with the specified endianness and the
	 * specified estimated data length using the specified factory. More
	 * accurate estimation yields less unexpected reallocation overhead. The new
	 ...
	 * specified {@code array}. A modification on the specified array's content
	 ...
	 * slice. A modification on the specified buffer's content will be visible
	...
	public static ChannelBuffer wrappedBuffer(ByteBuffer buffer) {
		...
		if (buffer.hasArray()) {
			return wrappedBuffer(buffer.order(), buffer.array(), buffer.arrayOffset() + buffer.position(),
					buffer.remaining());
		} else {
	...
	/**
	 * Creates a new buffer which wraps the specified buffer's readable bytes. A
	 * modification on the specified buffer's content will be visible to the
	 * returned buffer.
	 */
	 ...
	 * specified buffers without copying them. A modification on the content of
	 * the specified buffers will be visible to the returned buffer.
	 * 
	 ...
	 * Creates a new composite buffer which wraps the readable bytes of the
	 * specified buffers without copying them. A modification on the content of
	 * the specified buffers will be visible to the returned buffer. If
	 * gathering is {@code true} then gathering writes will be used when ever
	 * possible.
	 * 
	 * @throws IllegalArgumentException
	 *             if the specified buffers' endianness are different from each
				...
				for (ChannelBuffer c : buffers) {
					...
					if (c.readable()) {
						...
						if (c instanceof CompositeChannelBuffer) {
							...
							components
									.addAll(((CompositeChannelBuffer) c).decompose(c.readerIndex(), c.readableBytes()));
						} else {
	 ...
	 * Creates a new composite buffer which wraps the slices of the specified
	 * NIO buffers without copying them. A modification on the content of the
	 * specified buffers will be visible to the returned buffer. If gathering is
	 * {@code true} then gathering writes will be used when ever possible.
	 * 
	 * @throws IllegalArgumentException
	 *             if the specified buffers' endianness are different from each
	 *             other
	 ...
	 * {@code array}. The new buffer's {@code readerIndex} and
	...
	/**
	 * Creates a new buffer with the specified {@code endianness} whose content
	 * is a copy of the specified {@code array}. The new buffer's
	...
	/**
	 * Creates a new big-endian buffer whose content is a copy of the specified
	 * {@code array}'s sub-region. The new buffer's {@code readerIndex} and
	 * {@code writerIndex} are {@code 0} and the specified {@code length}
	 * respectively.
	 */
	...
	/**
	 * Creates a new buffer with the specified {@code endianness} whose content
	 * is a copy of the specified {@code array}'s sub-region. The new buffer's
	 * {@code readerIndex} and {@code writerIndex} are {@code 0} and the
	 ...
	 * {@code buffer}'s current slice. The new buffer's {@code readerIndex} and
	 * {@code writerIndex} are {@code 0} and {@code buffer.remaining}
	 * respectively.
	 */
	...
	/**
	 * Creates a new buffer whose content is a copy of the specified
	 * {@code buffer}'s readable bytes. The new buffer's {@code readerIndex} and
	 * {@code writerIndex} are {@code 0} and {@code buffer.readableBytes}
	 * respectively.
	 */
	...
	/**
	 * Creates a new big-endian buffer whose content is a merged copy of the
	 * specified {@code arrays}. The new buffer's {@code readerIndex} and
	 * {@code writerIndex} are {@code 0} and the sum of all arrays'
	...
	/**
	 * Creates a new buffer with the specified {@code endianness} whose content
	 * is a merged copy of the specified {@code arrays}. The new buffer's
	 * {@code readerIndex} and {@code writerIndex} are {@code 0} and the sum of
	 * all arrays' {@code length} respectively.
	 */
	 ...
	 * {@code buffers}' readable bytes. The new buffer's {@code readerIndex} and
	 ...
	 * {@code string} encoded in the specified {@code charset}. The new buffer's
	 * {@code readerIndex} and {@code writerIndex} are {@code 0} and the length
	...
	/**
	 * Creates a new big-endian buffer whose content is a subregion of the
	 * specified {@code string} encoded in the specified {@code charset}. The
	 * new buffer's {@code readerIndex} and {@code writerIndex} are {@code 0}
	...
	/**
	 * Creates a new buffer with the specified {@code endianness} whose content
	 * is the specified {@code string} encoded in the specified {@code charset}.
	...
	/**
	 * Creates a new buffer with the specified {@code endianness} whose content
	 * is a subregion of the specified {@code string} encoded in the specified
	 * {@code charset}. The new buffer's {@code readerIndex} and
	 * {@code writerIndex} are {@code 0} and the length of the encoded string
	 * respectively.
	 */
	public static ChannelBuffer copiedBuffer(ByteOrder endianness, CharSequence string, int offset, int length,
			Charset charset) {
		if (string == null) {
			throw new NullPointerException("string");
		}
		...
		if (string instanceof CharBuffer) {
			...
			if (buf.hasArray()) {
				return copiedBuffer(endianness, buf.array(), buf.arrayOffset() + buf.position() + offset, length,
						charset);
			}
	...
	/**
	 * Creates a new big-endian buffer whose content is the specified
	 * {@code array} encoded in the specified {@code charset}. The new buffer's
	 * {@code readerIndex} and {@code writerIndex} are {@code 0} and the length
	 * of the encoded string respectively.
	 */
	...
	/**
	 * Creates a new big-endian buffer whose content is a subregion of the
	 * specified {@code array} encoded in the specified {@code charset}. The new
	 * buffer's {@code readerIndex} and {@code writerIndex} are {@code 0} and
	 * the length of the encoded string respectively.
	 */
	...
	/**
	 * Creates a new buffer with the specified {@code endianness} whose content
	 * is the specified {@code array} encoded in the specified {@code charset}.
	 * The new buffer's {@code readerIndex} and {@code writerIndex} are
	 * {@code 0} and the length of the encoded string respectively.
	 */
	...
	/**
	 * Creates a new buffer with the specified {@code endianness} whose content
	 * is a subregion of the specified {@code array} encoded in the specified
	 * {@code charset}. The new buffer's {@code readerIndex} and
	 * {@code writerIndex} are {@code 0} and the length of the encoded string
	 * respectively.
	 */
	...
	/**
	 * @deprecated Use {@link #copiedBuffer(ByteOrder, CharSequence, Charset)}
	 *             instead.
	 */
	@Deprecated
	...
	/**
	 * Creates a read-only buffer which disallows any modification operations on
	 * the specified {@code buffer}. The new buffer has the same
	...
	/**
	 * Create a {@link ChannelBuffer} from the given <a
	 * href="http://en.wikipedia.org/wiki/Hex_dump">hex dump</a>
	 */
	public static ChannelBuffer hexDump(String hexString) {
		int len = hexString.length();
		byte[] hexData = new byte[len / 2];
		for (int i = 0; i < len; i += 2) {
			hexData[i / 2] = (byte) ((Character.digit(hexString.charAt(i), 16) << 4) + Character.digit(
	...
	/**
	 * Returns a <a href="http://en.wikipedia.org/wiki/Hex_dump">hex dump</a> of
	 * the specified buffer's readable bytes.
	 */
	...
	/**
	 * Returns a <a href="http://en.wikipedia.org/wiki/Hex_dump">hex dump</a> of
	 * the specified buffer's sub-region.
	 */
	...
	/**
	 * Calculates the hash code of the specified buffer. This method is useful
	 * when implementing a new buffer type.
	 */
	 ...
	 * {@code ChannelBuffer#equals(Object)}. This method is useful when
	 ...
	 * {@link ChannelBuffer#compareTo(ChannelBuffer)}. This method is useful
	 * when implementing a new buffer type.
	 */
	 ...
	 * {@link ChannelBuffer#indexOf(int, int, byte)}. This method is useful when
	 * implementing a new buffer type.
	 */
	...
	/**
	 * The default implementation of
	 * {@link ChannelBuffer#indexOf(int, int, ChannelBufferIndexFinder)}. This
	...
	private static int firstIndexOf(ChannelBuffer buffer, int fromIndex, int toIndex,
	...
	private static int lastIndexOf(ChannelBuffer buffer, int fromIndex, int toIndex,
			ChannelBufferIndexFinder indexFinder) {
		fromIndex = Math.min(fromIndex, buffer.capacity());
		if (fromIndex < 0 || buffer.capacity() == 0) {
			return -1;
