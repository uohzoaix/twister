...
//import com.twister.utils.internal.DetectionUtil;
...
public class CompositeChannelBuffer extends AbstractChannelBuffer {
	...
	/**
	 * Return {@code true} if gathering writes / reads should be used for this
	 * jdk >7 {@link CompositeChannelBuffer}
	 */
	...
	/**
	 * Same with {@link #slice(int, int)} except that this method returns a
	 * list.
	 */
	public List<ChannelBuffer> decompose(int index, int length) {
		...
		if (index + length > capacity()) {
			throw new IndexOutOfBoundsException("Too many bytes to decompose - Need " + (index + length)
	...
	public void getBytes(int index, byte[] dst, int dstIndex, int length) {
		if (index > capacity() - length || dstIndex > dst.length - length) {
			throw new IndexOutOfBoundsException("Too many bytes to read - Needs " + (index + length) + ", maximum is "
					+ capacity() + " or " + dst.length);
		}
		if (index < 0) {
	...
	public void getBytes(int index, ByteBuffer dst) {
		...
		if (index > capacity() - length) {
			throw new IndexOutOfBoundsException("Too many bytes to be read - Needs " + (index + length)
					+ ", maximum is " + capacity());
		}
		if (index < 0) {
			throw new IndexOutOfBoundsException("Index must be >= 0");
	...
	public void getBytes(int index, ChannelBuffer dst, int dstIndex, int length) {
		if (index > capacity() - length || dstIndex > dst.capacity() - length) {
			throw new IndexOutOfBoundsException("Too many bytes to be read - Needs " + (index + length) + " or "
					+ (dstIndex + length) + ", maximum is " + capacity() + " or " + dst.capacity());
		}
		if (index < 0) {
			throw new IndexOutOfBoundsException("Index must be >= 0");
		}
	...
	public void setBytes(int index, byte[] src, int srcIndex, int length) {
		...
		if (index > capacity() - length || srcIndex > src.length - length) {
			throw new IndexOutOfBoundsException("Too many bytes to read - needs " + (index + length) + " or "
					+ (srcIndex + length) + ", maximum is " + capacity() + " or " + src.length);
		}
	...
	public void setBytes(int index, ByteBuffer src) {
		...
		if (index > capacity() - length) {
			throw new IndexOutOfBoundsException("Too many bytes to be written - Needs " + (index + length)
					+ ", maximum is " + capacity());
		}
	...
	public void setBytes(int index, ChannelBuffer src, int srcIndex, int length) {
		...
		if (index > capacity() - length || srcIndex > src.capacity() - length) {
			throw new IndexOutOfBoundsException("Too many bytes to be written - Needs " + (index + length) + " or "
					+ (srcIndex + length) + ", maximum is " + capacity() + " or " + src.capacity());
		}
	...
	public int setBytes(int index, InputStream in, int length) throws IOException {
		int componentId = componentId(index);
		if (index > capacity() - length) {
			throw new IndexOutOfBoundsException("Too many bytes to write - Needs " + (index + length) + ", maximum is "
					+ capacity());
		}
	...
	public ChannelBuffer copy(int index, int length) {
		int componentId = componentId(index);
		if (index > capacity() - length) {
			throw new IndexOutOfBoundsException("Too many bytes to copy - Needs " + (index + length) + ", maximum is "
					+ capacity());
		}
	...
	/**
	 * Returns the {@link ChannelBuffer} portion of this
	 * {@link CompositeChannelBuffer} that contains the specified {@code index}.
	 * <strong>This is an expert method!</strong>
	 * 
	 * <p>
	 * Please note that since a {@link CompositeChannelBuffer} is made up of
	 * multiple {@link ChannelBuffer}s, this does <em>not</em> return the full
	 * buffer. Instead, it only returns a portion of the composite buffer where
	 * the index is located
	 * </p>
	 * 
	 * 
	 * @param index
	 *            The {@code index} to search for and include in the returned
	 *            {@link ChannelBuffer}
	 * @return The {@link ChannelBuffer} that contains the specified
	 *         {@code index}
	 * @throws IndexOutOfBoundsException
	 *             when the specified {@code index} is less than zero, or larger
	 *             than {@code capacity()}
	 */
	public ChannelBuffer getBuffer(int index) {
		if (index < 0 || index >= capacity()) {
			throw new IndexOutOfBoundsException("Invalid index: " + index + " - Bytes needed: " + index
					+ ", maximum is " + capacity());
		}
	...
	public ChannelBuffer slice(int index, int length) {
		if (index == 0) {
			...
			throw new IndexOutOfBoundsException("Invalid index: " + index + " - Bytes needed: " + (index + length)
					+ ", maximum is " + capacity());
		} else if (length == 0) {
			return ChannelBuffers.EMPTY_BUFFER;
		}
	...
	public ByteBuffer[] toByteBuffers(int index, int length) {
		if (index + length > capacity()) {
			throw new IndexOutOfBoundsException("Too many bytes to convert - Needs" + (index + length)
					+ ", maximum is " + capacity());
		}
		if (index < 0) {
			throw new IndexOutOfBoundsException("Index must be >= 0");
