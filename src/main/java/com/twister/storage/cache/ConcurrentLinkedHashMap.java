...
import static com.twister.storage.cache.ConcurrentLinkedHashMap.DrainStatus.IDLE;
import static com.twister.storage.cache.ConcurrentLinkedHashMap.DrainStatus.PROCESSING;
import static com.twister.storage.cache.ConcurrentLinkedHashMap.DrainStatus.REQUIRED;
...
public final class ConcurrentLinkedHashMap<K, V> extends AbstractMap<K, V> implements ConcurrentMap<K, V>, Serializable {
	...
	class ReadTask extends AbstractTask {
		...
		@GuardedBy("evictionLock")
		public void run() {
			// An entry may be scheduled for reordering despite having been
			// removed.
			// This can occur when the entry was concurrently read while a
			// writer was
			// removing it. If the entry is no longer linked then it does not
			// need to
			// be processed.
	...
	Set<K> orderedKeySet(boolean ascending, int limit) {
		...
		try {
			...
			int initialCapacity = (weigher == Weighers.entrySingleton()) ? Math.min(limit, (int) weightedSize()) : 16;
			Set<K> keys = new LinkedHashSet<K>(initialCapacity);
			Iterator<Node<K, V>> iterator = ascending ? evictionDeque.iterator() : evictionDeque.descendingIterator();
