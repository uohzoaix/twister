...
public final class ProductionMap<K, V> extends AbstractMap<K, V> implements ConcurrentMap<K, V>, Serializable {
	...
	public ProductionMap(EvictionPolicy policy, int initialCapacity, int concurrencyLevel, int maximumCapacity) {
		this.data = new ConcurrentHashMap<K, Node<K, V>>(initialCapacity, 0.75f, concurrencyLevel);
	...
	static final class Node<K, V> implements Serializable {
		...
		public void appendToTail() {
			...
			try {
				...
				// Read the tail on the stack to avoid unnecessary volatile
				// reads
		 ...
		 * <p>
		 * If the node has not yet been appended to the tail it will wait for
		 * that operation to complete.
		 */
		public void remove() {
			for (;;) {
				if (isUnlinked()) {
	...
	private final class KeyIterator implements Iterator<K> {
		private final EntryIterator iterator = new EntryIterator(ProductionMap.this.data.values().iterator());
